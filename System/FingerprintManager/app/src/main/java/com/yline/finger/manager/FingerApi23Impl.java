...
class FingerApi23Impl {
    ...
    void authenticate(Context context) {
        try {
            ...
            // 开始认证
            new FingerCompat().authenticate(context, cryptoObject, mSignal, new CallbackAdapter(mOnAuthCallback) {
                @Override
    ...
    void authenticateAdd(Context context, final String goodsInfo) {
        try {
            ...
            new FingerCompat().authenticate(context, object, mSignal, new CallbackAdapter(mOnAuthCallback) {
                @Override
                public void onAuthenticationSucceeded(FingerprintManagerCompat.AuthenticationResult result) {
                    if (null != mSignal) {
                        mSignal.cancel();
