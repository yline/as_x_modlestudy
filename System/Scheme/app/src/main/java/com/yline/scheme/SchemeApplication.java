...
public class SchemeApplication extends BaseApplication {
	// bitmap缓存
	...
	public void onCreate() {
		...
		// 大图片，磁盘缓存
		File parentPathFile = this.getExternalCacheDir();
		DiskCacheConfig mainDiskCacheConfig;
		if (null == parentPathFile) {
			mainDiskCacheConfig = DiskCacheConfig.newBuilder(this).build();
		} else {
			mainDiskCacheConfig = DiskCacheConfig.newBuilder(this)
		...
		Fresco.initialize(this, imagePipelineConfig);  // 初始化
