 ...
 * @version 1.0.0
 */
class SwipeFirstLayout : RelativeLayout {
    ...
    private fun initView() {
        val view = LayoutInflater.from(context).inflate(R.layout.view_swipe_first, this, true)

        val recyclerView = view.findViewById(R.id.view_swipe_first_recycler) as RecyclerView
        mSwipeAdapter = SwipeCommonAdapter(context)
        recyclerView.layoutManager = LinearLayoutManager(context)
        recyclerView.adapter = mSwipeAdapter

        mRefreshLayout = view.findViewById(R.id.view_swipe_first_refresh) as SwipeRefreshLayout
