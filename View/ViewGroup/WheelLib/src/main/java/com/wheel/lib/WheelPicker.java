...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
    ...
    private WheelPickerAttr mAttrHelper;
    ...
    private void computeTextSize() {
        ...
        if (mAttrHelper.isHasSameWidth()) {
            mTextMaxWidth = (int) mPaint.measureText(String.valueOf(mAttrHelper.getData(0)));
        } else if (isPosInRang(mAttrHelper.getTextMaxWidthPosition())) {
            mTextMaxWidth = (int) mPaint.measureText(String.valueOf(mAttrHelper.getData(mAttrHelper.getTextMaxWidthPosition())));
        } else if (!TextUtils.isEmpty(mAttrHelper.getMaxWidthText())) {
            mTextMaxWidth = (int) mPaint.measureText(mAttrHelper.getMaxWidthText());
        } else {
            for (Object obj : mAttrHelper.getData()) {
    ...
    private void computeFlingLimitY() {
        ...
        mMinFlingY = mAttrHelper.isCyclic() ? Integer.MIN_VALUE :
                -mItemHeight * (mAttrHelper.getData().size() - 1) + currentItemOffset;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mAttrHelper.getSelectedItemPosition(),
             drawnOffsetPos = -mHalfDrawnItemCount;
             drawnDataPos < drawnDataStartPos + mAttrHelper.getSelectedItemPosition() + mDrawnItemCount;
             drawnDataPos++, drawnOffsetPos++) {
            String data = "";
            if (mAttrHelper.isCyclic()) {
                int actualPos = drawnDataPos % mAttrHelper.getData().size();
                actualPos = actualPos < 0 ? (actualPos + mAttrHelper.getData().size()) : actualPos;
                data = String.valueOf(mAttrHelper.getData(actualPos));
            } else {
                if (isPosInRang(drawnDataPos)) {
                    data = String.valueOf(mAttrHelper.getData(drawnDataPos));
                }
            }
    ...
    private boolean isPosInRang(int position) {
        return position >= 0 && position < mAttrHelper.getData().size();
    }
    ...
    @Override
    public void run() {
        if (null == mAttrHelper.getData() || mAttrHelper.getData().size() == 0) {
            return;
        }
        if (mScroller.isFinished() && !isForceFinishScroll) {
            ...
            int position = (-mScrollOffsetY / mItemHeight + mAttrHelper.getSelectedItemPosition()) % mAttrHelper.getData().size();
            position = position < 0 ? position + mAttrHelper.getData().size() : position;
            if (isDebug) {
    ...
    public void setSelectedItemPosition(int position) {
        ...
        position = Math.min(position, mAttrHelper.getData().size() - 1);
    ...
    public void setMaximumWidthTextPosition(int position) {
        if (!isPosInRang(position)) {
            throw new ArrayIndexOutOfBoundsException("Maximum width text Position must in [0, " +
                    mAttrHelper.getData().size() + "), but current is " + position);
        }
