 ...
 * SwipeBack 使用类
 *
 * @author yline 2017/10/26 -- 10:50
 * @version 1.0.0
 */
public class SwipeBackManager implements SwipeBackCallback{
    private Activity mActivity;

    private SwipeBackLayout mSwipeBackLayout;

    public SwipeBackManager(Activity activity) {
        mActivity = activity;
    }
    ...
    public void convertActivityFromTranslucent() {
        try {
            Class<?> cls = null;
            Object obj = null;
            Method method = Activity.class.getDeclaredMethod("convertFromTranslucent", cls);
            method.setAccessible(true);
            method.invoke(mActivity, obj);
        } catch (Throwable t) {
        }
    }
